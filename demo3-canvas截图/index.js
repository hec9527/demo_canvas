...
// 以下代码为   获取canvas中的图片 并且截图保存到本地，或者再次绘制到页面

class CanvasSnap {
    constructor() {
        const drawClock = new DrawClock();

        this.canvas = drawClock.canvas;
        this.ctx = drawClock.ctx;
        this.btnBlob = document.getElementById('blob');
        this.btnDataURL = document.getElementById('dataURL');

        // 使用 toBlob API 获取屏幕截图  这个方法属于异步调用，使用会掉函数处理
        // 会掉函数第一个参数为 一个blob对象
        this.btnBlob.addEventListener(
            'click',
            e => {
                // void canvas.toBlob(callback, mimeType, quality);
                this.canvas.toBlob(
                    blob => {
                        // 必须使用这个方法创建一个短链接才能在 src/href 属性中使用
                        // 过多的资源可以使用 URL.revokeObjectURL 删除引用
                        const ulr = URL.createObjectURL(blob);
                        const el = document.createElement('a');

                        el.download = 'canvas截图-toBlob';
                        el.href = ulr;
                        el.click();
                    },
                    'image/png',
        ...
        this.btnDataURL.addEventListener(
            'click',
            e => {
                // canvas.toDataURL([mimeType, quality]);
                const imgData = this.canvas.toDataURL('image/png', 96);
                const el = document.createElement('a');

                console.log(1);

                el.download = 'canvas截图-toDataURL';
                el.href = imgData;
                el.click();
            },
            false
