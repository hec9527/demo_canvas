...
class DrawClock {
    ...
    drawCircle() {
        ...
        this.ctx.beginPath();
        this.ctx.arc(this.canvas.width / 3, this.canvas.height / 2, this.RADIUS, 0, Math.PI * 2, true);
        this.ctx.stroke();
        this.ctx.restore();
    }
    ...
    drawNumbers() {
        ...
        numbers.forEach(num => {
            ...
            this.ctx.fillText(
                num,
                this.canvas.width / 3 + Math.cos(angle) * this.HAND_RADIUS - numberWidth / 2,
    ...
    drawCenter() {
        this.ctx.beginPath();
        this.ctx.arc((this.canvas.width / 3) | 0, (this.canvas.height / 2) | 0, 5, 0, Math.PI * 2, true);
        this.ctx.fill();
    }
    ...
    drawHand(ioc, isHour) {
        ...
        const handRadius = isHour ? this.RADIUS - this.HAND_TRUNCATION - this.HOUR_HAND_TRUNCATION : this.RADIUS - this.HAND_TRUNCATION;
    ...
    drwaDateNumber() {
        ...
        this.ctx.fillText(title, ((this.canvas.width / 5) * 4 - titleSize.width / 2) | 0, (this.canvas.height / 2 - 50) | 0);

        this.ctx.font = 36 + 'px Arial';
        const hour = `${date.getHours()} : ${date.getMinutes()} : ${date.getSeconds()}`;
        const hourSize = this.ctx.measureText(hour);
        this.ctx.fillText(hour, ((this.canvas.width / 5) * 4 - hourSize.width / 2) | 0, this.canvas.height / 2);

        this.ctx.font = 15 + 'px Arial';
        const weekDay = `星期${date.getDay()}`;
        const weekDaySize = this.ctx.measureText(weekDay).width;
        this.ctx.fillText(weekDay, ((this.canvas.width / 5) * 4 - weekDaySize / 2) | 0, this.canvas.height / 2 + 35);
